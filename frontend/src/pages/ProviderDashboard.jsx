            ...
            <main className="max-w-7xl mx-auto px-4 py-32">
                <div className="mb-12 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div>
                        ...
                        <p className="text-gray-500 text-lg font-medium">Grow your business and manage your schedule.</p>
                    </div>
                    {activeTab === 'services' && (
                        <Button onClick={() => navigate('/provider/create-listing')} size="lg" className="rounded-2xl h-16 px-8 shadow-xl shadow-indigo-100 flex items-center gap-3">
                ...
                <div className="flex gap-2 p-1.5 glass rounded-[2rem] w-fit mb-12 shadow-inner overflow-x-auto max-w-full">
                    {[
                        ...
                        <button
                            ...
                            className={`flex items-center gap-3 px-8 py-3.5 rounded-2xl text-sm font-black transition-all duration-300 whitespace-nowrap ${activeTab === tab.id ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-100' : 'text-gray-400 hover:text-indigo-600 hover:bg-white/50'}`}
                        >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {loading && listings.length === 0 ? (
                            <div className="col-span-full py-32 glass rounded-[3rem] text-center border-dashed">
                                ...
                                <p className="font-bold text-gray-400">Fetching your services...</p>
                            </div>
                        ) : listings.length > 0 ? (
                            listings.map(listing => (
                                <div key={listing.id} className="glass group overflow-hidden rounded-[2.5rem] transition-all duration-500 hover:scale-[1.02] border-white/60">
                                    <div className={`h-56 relative overflow-hidden ${listing.disabled ? 'grayscale' : ''}`}>
                                        <img src={listing.images} alt={listing.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                                        <div className="absolute top-4 right-4 flex gap-2">
                                            <button onClick={() => navigate(`/provider/update-listing/${listing.id}`)} className="p-3 bg-white/90 backdrop-blur-sm text-indigo-600 rounded-xl shadow-lg hover:bg-white transition-colors">
                                                <Edit size={18} />
                                            </button>
                                            <button onClick={() => handleDeleteListing(listing.id)} className="p-3 bg-white/90 backdrop-blur-sm text-rose-600 rounded-xl shadow-lg hover:bg-white transition-colors">
                                                <Trash2 size={18} />
                                            </button>
                                        </div>
                                    </div>
                                    <div className="p-8">
                                        <div className="flex justify-between items-start mb-4">
                                            <h3 className="text-2xl font-black text-gray-900 tracking-tight leading-tight">{listing.title}</h3>
                                            <span className="text-2xl font-black text-indigo-600 tracking-tighter">₹{listing.price}</span>
                                        </div>
                                        <p className="text-gray-500 text-sm line-clamp-2 mb-8 font-medium leading-relaxed">{listing.description}</p>
                                        <div className="flex items-center gap-3 pt-6 border-t border-white/40">
                                            <Button variant={listing.disabled ? 'primary' : 'outline'} size="sm" className="w-full text-xs font-black uppercase tracking-widest h-11" onClick={() => {
                                                const updated = listings.map(l => l.id === listing.id ? { ...l, disabled: !l.disabled } : l);
                                                setListings(updated);
                                            }}>
                            ...
                            <div className="col-span-full py-32 glass rounded-[3rem] text-center border-dashed">
                                ...
                                <h3 className="text-2xl font-black text-gray-900">No Services Yet</h3>
                                <p className="text-gray-400 mt-2 font-medium">Create your first listing to start accepting bookings.</p>
                            </div>
                        )}
                    </div>
                )}
                    ...
                    <div className="space-y-8">
                        {loading ? (
                            <div className="py-32 glass rounded-[3rem] text-center">
                                <div className="w-12 h-12 border-4 border-indigo-600/20 border-t-indigo-600 rounded-full animate-spin mx-auto mb-6" />
                                <p className="font-bold text-gray-400">Loading appointments...</p>
                            </div>
                        ) : bookings.length === 0 ? (
                            <div className="py-32 glass rounded-[3rem] text-center border-dashed">
                                <Calendar size={64} className="mx-auto text-gray-200 mb-6" />
                                <h3 className="text-2xl font-black text-gray-900">Queue is empty</h3>
                                <p className="text-gray-400 mt-2 font-medium">Upcoming bookings will be listed here.</p>
                            </div>
                        ) : (
                            bookings.map(booking => (
                                <div key={booking.bookingId} className="glass p-8 rounded-[2.5rem] flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8 border-white/60 hover:shadow-2xl transition-all duration-500">
                                    <div className="flex-1">
                                        <div className="flex flex-wrap items-center gap-4 mb-4">
                                            <h3 className="text-2xl font-black text-gray-900 leading-tight">{booking.serviceName}</h3>
                                            <span className={`px-4 py-1.5 text-[10px] rounded-full font-black uppercase tracking-widest border border-white/40 shadow-inner ${booking.status === 'CONFIRMED' ? 'bg-emerald-100 text-emerald-700' : booking.status === 'PENDING' ? 'bg-amber-100 text-amber-700' : 'bg-indigo-100 text-indigo-700'}`}>
                                                {booking.status}
                                            </span>
                                        </div>
                                    ...
                                    <div className="flex gap-3 w-full lg:w-auto mt-6 lg:mt-0 pt-8 lg:pt-0 border-t lg:border-t-0 border-white/40">
                                        {booking.status === 'PENDING' && (
                                            <>
                                                <Button size="lg" className="flex-1 lg:flex-none rounded-2xl h-14 px-8" onClick={() => handleConfirmBooking(booking.bookingId)}>Accept</Button>
                                                <Button variant="outline" size="lg" className="flex-1 lg:flex-none rounded-2xl h-14 px-8 text-rose-600 border-rose-100" onClick={() => handleProviderCancelBooking(booking.bookingId)}>Decline</Button>
                                            </>
                                        )}
                                        {booking.status === 'CONFIRMED' && (
                                            <>
                                                <Button size="lg" className="flex-1 lg:flex-none rounded-2xl h-14 px-8" onClick={() => handleCompleteBooking(booking.bookingId)}>Complete Job</Button>
                                                <Button variant="outline" size="lg" className="flex-1 lg:flex-none rounded-2xl h-14 px-8" onClick={() => handleProviderCancelBooking(booking.bookingId)}>Cancel</Button>
                                            </>
                                        )}
                    ...
                    <div className="space-y-8">
                        {loading && listings.length === 0 ? (
                            ...
                            <div className="py-32 glass rounded-[3rem] text-center border-dashed border-white/60">
                                ...
                                <p className="text-gray-400 mt-2 mb-8 font-medium max-w-md mx-auto">You need to create a service listing before you can set your availability.</p>
                                <Button
                            ...
                            <div className="glass p-8 rounded-[2.5rem] border-white/60">
                                ...
                                <div className="mb-8 p-6 bg-indigo-50/50 rounded-3xl border border-indigo-100">
                                    <label className="text-xs font-black text-indigo-400 uppercase tracking-widest mb-3 block">Select Service to Manage</label>
                                    <select
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                                    ...
                                    <div>
                                        <div className="flex justify-between items-center mb-6">
                                            <h3 className="text-2xl font-black text-gray-900">Time Slots ({selectedDay})</h3>
                                            <Button
                                                onClick={async () => {
                                                    setSavingAvailability(true);
                                                    try {
                                                        await updateProviderAvailability(userId, selectedDay, availability);
                                                        ...
                                                        return (
                                                            ...
                                                            >
                                                                <div className="flex items-center gap-2">
                                                                    <Clock size={18} className={isActive ? 'text-indigo-200' : 'text-indigo-400'} />
                                                                    {slot}
                                                                </div>
                ...
                {activeTab === 'earnings' && (
                    <div className="space-y-12">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                            {[
                                { label: 'TOTAL EARNED', value: '₹0.00', icon: TrendingUp, color: 'text-indigo-600' },
                                { label: 'PENDING PAYOUT', value: '₹0.00', icon: DollarSign, color: 'text-amber-600' },
                                { label: 'HAPPY CLIENTS', value: '0', icon: Users, color: 'text-emerald-600' },
                            ].map(stat => (
                                <div key={stat.label} className="glass p-8 rounded-[2.5rem] shadow-xl border-white/60 relative overflow-hidden group">
                        ...
                        <div className="py-32 glass rounded-[3rem] text-center border-dashed">
                            ...
                            <p className="text-gray-400 font-black text-xl">Financial analytics coming soon</p>
                        </div>
                    </div>
                )}
...
const CalendarWidget = ({ selectedDateUI, setSelectedDateUI, selectedDay, setSelectedDay }) => {
    ...
    return (
        <div>
            ...
            <div className="bg-white/40 rounded-3xl p-6 border border-white/40">
                <div className="flex justify-between items-center mb-6">
                    <button onClick={() => setCurrDate(new Date(year, month - 1))} className="p-2 hover:bg-white rounded-xl"><ChevronRight className="rotate-180" /></button>
                    <span className="font-black text-lg text-gray-700">{monthNames[month]} {year}</span>
                    <button onClick={() => setCurrDate(new Date(year, month + 1))} className="p-2 hover:bg-white rounded-xl"><ChevronRight /></button>
                </div>
                ...
                <div className="grid grid-cols-7 gap-2">
                    ...
                    {days.map(d => {
                        ...
                        const isUISelected = selectedDateUI && selectedDateUI.toDateString() === dateObj.toDateString();
            ...
            <div className="mt-6 p-4 bg-amber-50 rounded-2xl border border-amber-100 flex gap-3 text-amber-800 text-sm font-medium">
                ...
                <p>Setting availability for <span className="font-black underline">{selectedDay}</span>. This schedule repeats weekly.</p>
            </div>
        </div>
    );
};
